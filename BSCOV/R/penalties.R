...
ssic.penalty <- function(T,cpt,alpha=0.5,ssic.type=c("power")){
	alpha <- as.numeric(alpha)

	ssic.type <- match.arg(ssic.type)

	if (ssic.type == "log")  pen <- log(T)^alpha
	else if (ssic.type == "power") pen <- T^alpha
...
mbic.penalty <- function(T,cpt){
	k <- length(cpt)
	if(k>0)	3/2 * k * log(T) + 1/2 * sum(log(diff(c(0,sort(cpt),T))/T))
	else 1/2 * sum(log(diff(c(0,T))/T))
}
