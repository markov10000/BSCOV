...
if(input.op==3){#wavelet
    ...
    para<-list(M=M, SN.op=SN.op, is.NoF.fixed=r, max.q=max.q, sig.lev=sig.lev, dw=dw, rule=rule, normalisation=normalisation, refact.op=refact.op,
    ...
    if(q.hat){#NoF>0
        ...
        hat.vep<-get.residual(gfm0=gfm0, common.est.cps=common.est.cps, refact=refact.op,bn.op = bn.op)
   }else{#NoF=0
        cs <- NULL
